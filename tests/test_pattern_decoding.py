...
class TestDeBruijn:
    ...
    def test_decode_color_sequence(self):
        ...
        idx, valid = self.debruijn.decode_color_sequence(
            red_patch_noisy, color_tolerance=20
        )
        assert idx == 0
        assert valid == True
        ...
        idx, valid = self.debruijn.decode_color_sequence(
            invalid_patch, color_tolerance=20
        )
        assert valid == False
    ...
    def test_decode_mode_xy(self):
        ...
        forward_map_xy, fg_xy = self.debruijn.decode(captures, self.proj_wh, mode="xy")

        forward_map_ij, fg_ij = self.debruijn.decode(captures, self.proj_wh, mode="ij")
...
class TestPhaseShifting:
    ...
    def test_encode(self):
        ...
        # Check output shape
        expected_images = (
            2 * self.phase_shifting.num_phases + 2
...
class TestComparisonWithGrayCode:
    ...
    def test_pattern_count_comparison(self):
        ...
        assert (
            db_count < gc_count
        ), f"DeBruijn ({db_count}) should require fewer patterns than GrayCode ({gc_count})"
        assert (
            ps_count < gc_count
        ), f"PhaseShifting ({ps_count}) should require fewer patterns than GrayCode ({gc_count})"

        print(
            f"Pattern counts: GrayCode={gc_count}, DeBruijn={db_count}, PhaseShifting={ps_count}"
        )
